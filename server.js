...
bot.on("messageCreate", message => {
    ...
    Server.findOrCreate({where: {id: message.guild.id}, defaults: {id: message.guild.id, name: message.guild.name}}).then(serv => { //returns an array. find just one?
        ...
        var temparray = [956353559320985600,442177059037511692,972811971718164531,963431307394818098,956758915142541354,970012891874983966,825789671631355975,980988755974455307,591505589696331787,990546935045820436,982795624191770624,843944695591337995,633326752617463829,995298917447106611,995752147893375056,972693198960992307,999197175902908466,429060734669029397,844775751466352671,984496786284613644,993198419709538335,316751925968109578,809695577629261824,513389283994763266,931943755378983022,1001114918092816384,820463899358527548,993827756704010260,410658436989190153,1003446033969332274,381532839704920066,978161178788364359,999484926443409469,792446893958496276,990581461923033118,717419439397798000,893264863529406505,720161942714581032]

        if (msg == "bierce") {
            // temparray.forEach(s => {
            //     Bowl.count({where: {serverId: s}}).then(b=>console.log(s +" = " +b))
            // })
            for (let i = 0; i < temparray.length; i++) {
                Bowl.count({where: {serverId: temparray[i]}}).then(b=>console.log(temparray[i] +" = " +b))
            }
        }
